...
@interface TWBezierViewController ()<CAAnimationDelegate>
...
@implementation TWBezierViewController
...
- (void)addBezierPath
{
    ...
    round2.centerX = self.view.centerX;
    round2.centerY = self.view.centerY;
...
- (void) startAnim
{
    ...
    [self viewMovewPathAnimatWith:_round1 path:path1 andTime:_animTime];
    [self viewColorAnmiatWith:_round1 fromColor:_round1Color toColor:_round3Color andTime:_animTime];
    
    UIBezierPath *path2 = [[UIBezierPath alloc] init];
    [path2 addArcWithCenter:otherRoundCenter1 radius:10 startAngle:0 endAngle:-M_PI clockwise:true];
    [self viewMovewPathAnimatWith:_round2 path:path2 andTime:_animTime];
    [self viewColorAnmiatWith:_round2 fromColor:_round2Color toColor:_round1Color andTime:_animTime];
    ...
    [self viewMovewPathAnimatWith:_round3 path:path3 andTime:_animTime];
    [self viewColorAnmiatWith:_round3 fromColor:_round3Color toColor:_round1Color andTime:_animTime];

    
}

//设置view的移动路线，这样抽出来因为每个圆只有路径不一样
- (void)viewMovewPathAnimatWith:(UIView *)view path:(UIBezierPath *)path andTime:(CGFloat)time
{
...
- (void)viewColorAnmiatWith:(UIView *)view fromColor:(UIColor *)fromColor toColor:(UIColor *)toColor andTime:(CGFloat)time
{
    ...
    [view.layer addAnimation:colorAnim forKey:@"backgroudColor"];
